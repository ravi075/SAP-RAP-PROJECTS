...
CLASS lhc_user IMPLEMENTATION.
  ...
  READ ENTITIES OF zi_user IN LOCAL MODE ENTITY User
  ...
  METHOD downloadexcel.
   ...
   DATA: lt_template        TYPE STANDARD TABLE OF zbp_i_user=>gty_exl_file.
...
*  "Modify Root Entity
    MODIFY ENTITIES OF zi_user IN LOCAL MODE
    ENTITY User
...
*    "Read Updated Entry
    READ ENTITIES OF zi_user IN LOCAL MODE
    ENTITY  User
    ALL FIELDS WITH
    CORRESPONDING #( Keys )
    RESULT DATA(lt_User).
*    "Update File Status
  ...
  METHOD uploadexceldata.
    ...
    DATA: lt_rows         TYPE STANDARD TABLE OF string,
          ...
          lt_excel        TYPE STANDARD TABLE OF zbp_i_user=>gty_exl_file,
          lt_excel_temp   TYPE STANDARD TABLE OF zbp_i_user=>gty_exl_file,
          lt_excel_filter TYPE SORTED TABLE OF zbp_i_user=>gty_exl_file WITH UNIQUE KEY emp_id dev_id,
          lt_data         TYPE TABLE FOR CREATE zi_user\_UserDev,
